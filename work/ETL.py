...
def clean_data(data):
    ...
    #We'll exclude the offence against a person when we deal with anything featuring location. - Comments made by Ryan, code by Armin.
    data.loc[data['X'] < 100000, "X"] = meanX
    data.loc[data['Y'] < 100000, "Y"] = meanY
    
    #convert the date columns to a proper datetime - coded by Ryan. It's actually surprisingly performant.
